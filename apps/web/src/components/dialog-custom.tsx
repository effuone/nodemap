...
import getGPTDescription from "@/api/getGPTDescription";
...
export default function DialogCustom({
  ...
  useEffect(() => {
    if (isOpen && title) {
      ...
      getGPTDescription(title).then((response) => {
            if (response) {
                setInfo(response.choices[0].message.content);
